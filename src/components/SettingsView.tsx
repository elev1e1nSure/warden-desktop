...
function AgentSection({
  ...
  const toggleSystemPrompt = async () => {
    ...
    setBusy(true);
    setSettingsState({ ...settings, disable_system_prompt: next });
    try {
  ...
  return (
    <>
      ...
      <FieldGroup>
        ...
        >
          <Toggle
            checked={Boolean(settings?.disable_system_prompt)}
            onChange={toggleSystemPrompt}
        ...
        {PERMISSION_LEVELS.map((level) => {
          ...
          return (
            <button
              ...
              disabled={disabled}
              onClick={() => {
                if (!active) onChange(level.value);
              }}
              style={{ isolation: "isolate" }}
