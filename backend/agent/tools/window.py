...
class WindowListTool(Tool):
    ...
    params = {
        "filter": {
            "type": "string",
            "description": "Substring to filter by window title (optional)",
        },
    }
...
class WindowFocusTool(Tool):
    ...
    params = {
        "title": {
            "type": "string",
            "description": "Window title substring (optional if hwnd given)",
        },
        "hwnd": {
            "type": "integer",
            "description": "Window handle from window_list (optional if title given)",
        },
    }
    ...
    async def execute(self, args: dict[str, Any]) -> str:
        ...
        try:
            ...
            if action in self._SHOW:
                ...
                body = (
                    f"[void][WardenWin]::MoveWindow([IntPtr]{handle}, {x}, {y}, {w}, {h}, $true)\n"
                )
            else:
