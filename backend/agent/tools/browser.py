...
class BrowserReadTool(Tool):
    ...
    async def execute(self, args: dict[str, Any]) -> str:
        ...
        try:
            ...
            if data["links"]:
                out += "\n\nLinks:\n" + "\n".join(
                    f"• {l['text']}: {l['url']}" for l in data["links"]
                )
            return out[:3000]
        except ImportError:
...
class BrowserScreenshotTool(Tool):
    ...
    description = (
        "Take a screenshot of a web page in the background via Playwright and return the file path."
    )
    params = {"url": {"type": "string", "description": "URL"}}
...
class BrowserClickTool(Tool):
    ...
    params = {
        "selector": {
            "type": "string",
            "description": "CSS/XPath selector or visible text to click",
        },
        "url": {"type": "string", "description": "URL to navigate to first (optional)"},
        "timeout": {
            "type": "integer",
            "description": "Timeout in seconds for the action (default 15)",
        },
    }
...
class BrowserFillTool(Tool):
    ...
    params = {
        "selector": {
            "type": "string",
            "description": "CSS/XPath selector or label/placeholder text of the field",
        },
