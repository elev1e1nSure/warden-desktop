...
def _file_to_lang(path: str) -> tuple[str | None, str | None, str | None, str | None]:
...
class _LspClient:
        ...
        await asyncio.wait_for(
            self._request(
                "initialize",
                {
                    ...
                    "trace": "off",
                    "workspaceFolders": [
                        {"uri": root_uri, "name": Path(root_uri.replace("file://", "")).name}
                    ],
                },
            ),
    ...
    async def references(
        self, uri: str, line: int, char: int, include_decl: bool = True
    ) -> list[dict]:
        return await self._call(
