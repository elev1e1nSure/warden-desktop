...
class ImageLocateTool(Tool):
    ...
    async def execute(self, args: dict[str, Any]) -> str:
        ...
        return (
            f"found at ({mx}, {my}) — size {int(box.width)}x{int(box.height)}, screen ({rx}, {ry})"
        )
...
async def _ocr_image(path: str) -> str:
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            (proc.stderr or "OCR failed").strip().splitlines()[0] if proc.stderr else "OCR failed"
        )
...
class OcrTool(Tool):
    ...
    params = {
        "image": {
            "type": "string",
            "description": "Path to an image to OCR (optional; defaults to a screen grab)",
        },
...
class WaitForTool(Tool):
    ...
    params = {
        ...
        "target": {
            "type": "string",
            "description": "Window title substring, text to find, or template image path",
        },
    ...
    async def _check(self, kind: str, target: str) -> bool:
        if kind == "window":
            ...
            needle = target.lower()
            return any(
                needle in str(w.get("title", "")).lower() for w in await _enumerate_windows()
            )
        if kind == "image":
            import pyautogui
            ...
            def _locate():
                try:
                    ...
                    if (
                        "ImageNotFound" in type(e).__name__
                        or "could not be located" in str(e).lower()
                    ):
                        return None
                    raise
