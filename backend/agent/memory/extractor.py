...
class MemoryExtractor:
    ...
    _TRIGGERS = [
        # tech_stack
        (
            re.compile(r'(?:tech\s*stack|stack)["\'\s:]*([^,.;]{1,60})', re.IGNORECASE),
            "tech_stack",
            "stack",
        ),
        (
            re.compile(
                r'(?:using|use|на\s*\w+\s*написано\s*на|использу[юем]|стек)["\'\s:]*([^,.;]{1,60})',
                re.IGNORECASE,
            ),
            "tech_stack",
            "stack",
        ...
        (
            re.compile(
                r'(?:prefer|like|want|love|предпочитаю|люблю|хочу)["\'\s:]*([^,.;]{1,60})',
                re.IGNORECASE,
            ),
            "preference",
            "preference",
        ...
        (
            re.compile(r'(?:style|стиль)["\'\s:]*([^,.;]{1,60})', re.IGNORECASE),
            "preference",
            "style",
        ),
        # project
        (
            re.compile(
                r'(?:project|проект)["\'\s:]*(?:name|название)?["\'\s:]*([^,.;]{1,60})',
                re.IGNORECASE,
            ),
            "project",
            "name",
        ...
        (
            re.compile(r'(?:работаю\s+над|working\s+on)["\'\s:]*([^,.;]{1,60})', re.IGNORECASE),
            "project",
            "name",
        ),
        # user
        (
            re.compile(r'(?:my\s+name\s+is|меня\s+зовут)["\'\s:]*([^,.;]{1,60})', re.IGNORECASE),
            "user",
            "name",
        ),
        (
            re.compile(r'(?:я\s+|I["\'\s]m\s+)([A-Z][a-z]{1,20})(?:\s|$|[,.])', re.IGNORECASE),
            "user",
            "name",
        ),
        (
            re.compile(r'(?:preferred\s+language|язык)["\'\s:]*([^,.;]{1,30})', re.IGNORECASE),
            "user",
            "preferred_language",
        ),
    ]
