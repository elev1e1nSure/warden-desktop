		...
		with open(dest, "wb") as f:
			while True:
				...
				if not chunk:
					break
				f.write(chunk)
		content_type = field.headers.get("Content-Type", "application/octet-stream") if hasattr(field, "headers") else "application/octet-stream"
		files.append({
			"id": safe_name,
			"name": filename,
			"type": content_type,
...
async def chat(request: web.Request) -> web.StreamResponse:
	...
	if backend.chat is None:
		await response.write(json.dumps({"type": "error", "text": "not connected — run /connect to get started"}, ensure_ascii=False).encode() + b"\n")
